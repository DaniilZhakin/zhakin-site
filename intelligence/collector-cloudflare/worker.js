...
export function validateEvent(event) {
  ...
  if (event.timestamp !== undefined &&
      (typeof event.timestamp !== 'string' || event.timestamp.length > 64 || Number.isNaN(Date.parse(event.timestamp)))) {
    return 'invalid timestamp';
  }
  if (event.referrer_class !== undefined &&
      (typeof event.referrer_class !== 'string' || event.referrer_class.length > 32 || !REFERRER_CLASSES.has(event.referrer_class))) {
    return 'invalid referrer_class';
  }
  if (event.content_id !== undefined &&
      (typeof event.content_id !== 'string' || event.content_id.length > 128 || !/^[A-Za-z0-9._-]+$/.test(event.content_id))) {
...
function getAllowedOrigins(env) {
  if (env.ALLOWED_ORIGIN) return new Set([env.ALLOWED_ORIGIN, ...ALLOWED_ORIGINS]);
...
function corsHeaders(request, env) {
  ...
  if (origin && getAllowedOrigins(env).has(origin)) headers['access-control-allow-origin'] = origin;
  return headers;
}
...
export async function purgeExpiredAggregates(env, now = new Date()) {
  ...
  const result = await env.DB.prepare('DELETE FROM engagement_daily WHERE event_day < ?').bind(cutoff).run();
...
export default {
  async fetch(request, env) {
    ...
    if (!isAllowedOrigin(request, env)) return json({ error: 'cors_forbidden' }, 403, cors);

    const url = new URL(request.url);
    if (url.pathname !== '/api/events') return json({ error: 'not_found' }, 404, cors);
    ...
    if (!rate.allowed) return json({ error: 'rate_limited' }, 429, { ...cors, 'retry-after': String(rate.retry_after) });

    const raw = await request.text();
    if (!validateRequestSize(request, raw)) return json({ error: 'payload_too_large' }, 413, cors);

    let payload;
    try { payload = JSON.parse(raw); } catch { return json({ error: 'invalid_json' }, 400, cors); }

    const events = Array.isArray(payload) ? payload : [payload];
    if (events.length < 1 || events.length > MAX_EVENTS_PER_REQUEST) return json({ error: 'invalid_event_count' }, 400, cors);
    ...
    try { await aggregateEvents(env, events, receivedAt); }
    catch { return json({ error: 'storage_unavailable' }, 503, cors); }

    return json({ accepted: events.length, received_at: receivedAt }, 202, cors);
  },
  async scheduled(controller, env) { await purgeExpiredAggregates(env, new Date(controller.scheduledTime)); },
};
